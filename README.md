...
- **Empty `@Schema()` classes are permissive:** `@Schema()` on a class with no `@Is*` fields builds `z.object({})` in default strip mode, which accepts any input and drops unknown keys. If you want `@Schema()` to imply "reject unknown keys", add fields or configure the class with `strict` unknown keys via a custom setup. The pipe and `toZodSchema` both treat such classes as valid schema sources (not passthrough).
- **Caching:** `validate` / `validateSafe` rebuild the schema on every call (prototype walk + shape assembly). The NestJS `ZodValidationPipe` caches derived schemas in a module-level `WeakMap` keyed by the DTO constructor. If you use `validate` / `validateSafe` in hot paths, cache the result of `toZodSchema(cls)` yourself.
...
**Error bodies:** the default `BadRequestException` response includes `errors` with Zod issue metadata but **without** `input` / `received` / `params` / `keys` (strict-object unknown keys), so raw request values, custom `addIssue` payloads, and client-supplied extra property names are not echoed. Use `redactZodIssuesForResponse` in a custom `errorFactory` if you build your own payload from `error.issues`. A custom `errorFactory` must **return** an `Error` (e.g. `new BadRequestException(...)` or `throw` inside the factory); returning a non-`Error` throws a `TypeError` (misconfiguration). With `transform: true`, any `TypeError` from `plainToInstance` is turned into a 400 with the same `message: "Validation failed"` shape. `ZodValidationPipe` accepts `maxTransformDepth` (default 512; use `Number.POSITIVE_INFINITY` to disable the cap; non-finite or negative values fall back to 512) and passes it to `plainToInstance`.
