...
**Error bodies:** the default `BadRequestException` response includes `errors` with Zod issue metadata but **without** `input` / `received` / `params`, so raw request values and custom `addIssue` payloads are not echoed. Use `redactZodIssuesForResponse` in a custom `errorFactory` if you build your own payload from `error.issues`.
